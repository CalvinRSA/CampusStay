...
export default function AdminDashboard() {
  ...
  const handleAddProperty = async (e: FormEvent<HTMLFormElement>) => {
    ...
    formData.append('campus_intake', addForm.campus_intake);
  ...
  const handleEdit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    formData.append('campus_intake', editForm.campus_intake);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
            ...
            <div className="px-6 py-4 bg-gray-50 flex gap-3 justify-end rounded-b-xl">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg font-medium transition ${
                  confirmDialog.type === 'danger' ? 'bg-red-600 hover:bg-red-700 text-white' :
                  confirmDialog.type === 'warning' ? 'bg-yellow-600 hover:bg-yellow-700 text-white' :
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <button 
              ...
              className="flex items-center gap-2 bg-orange-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-orange-700 transition shadow-sm"
            >
              ...
              <div key={p.id} className="bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-md transition">
                {p.image_urls[0] ? (
                  <div className="h-48 overflow-hidden">
                    <img src={`http://localhost:8000${p.image_urls[0]}`} alt={p.title} className="w-full h-full object-cover" />
                  </div>
                ) : (
                ...
                <div className="p-5">
                  ...
                  <p className="text-xs text-gray-500 mb-4">Campus: {p.campus_intake}</p>
                  ...
                  <div className="bg-gray-50 rounded-lg p-4 mb-4">
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div 
                        className={`h-2 rounded-full transition-all ${p.available_flats > 0 ? 'bg-green-500' : 'bg-gray-400'}`}
                        style={{ width: `${(p.available_flats / p.total_flats) * 100}%` }}
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <button 
                      onClick={() => openEdit(p)} 
                      className="flex-1 bg-blue-600 text-white py-2 rounded-lg font-medium hover:bg-blue-700 transition flex items-center justify-center gap-2 text-sm"
                    >
      ...
      {/* Applications Tab - Pending Only */}
      {activeTab === 'applications' && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">Pending Applications</h2>
          {loading ? (
            <div className="text-center py-20 text-gray-600">Loading applications...</div>
              ...
              {pendingApplications.map(app => (
                <div key={app.id} className="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition">
                  <div className="grid md:grid-cols-3 gap-6">
                    <div>
                      <h3 className="text-lg font-bold text-gray-900 mb-2">{app.student_name}</h3>
                    ...
                    <div>
                      <h4 className="text-sm font-semibold text-gray-600 mb-1">Applied For:</h4>
                      <p className="text-lg font-bold text-orange-600">{app.property_title}</p>
                      <p className="text-sm text-gray-500 mt-2">Applied {new Date(app.applied_at).toLocaleDateString('en-ZA', { year: 'numeric', month: 'short', day: 'numeric' })}</p>
                    ...
                    <div className="flex flex-col justify-center items-end gap-3">
                      ...
                      <div className="flex flex-col gap-2 w-full">
                        <button 
                          ...
                          className="bg-blue-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-blue-700 transition flex items-center justify-center gap-1.5 text-sm w-full"
                        >
                        ...
                        <div className="flex gap-2">
                          <button 
                            ...
                            className="flex-1 bg-green-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-green-700 transition flex items-center justify-center gap-1.5 text-sm"
                          >
                            <CheckCircle className="w-4 h-4" /> Approve
                          </button>
                          <button 
                            onClick={() => handleApplicationStatus(app.id, 'rejected')} 
                            className="flex-1 bg-red-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-red-700 transition flex items-center justify-center gap-1.5 text-sm"
                          >
      ...
      {activeTab === 'analytics' && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          {!selectedPropertyForAnalytics ? (
            ...
            <>
              {(() => {
                const propertyApps = applications.filter(app => app.property_id === selectedPropertyForAnalytics);
                ...
                const pending = propertyApps.filter(app => app.status === 'pending').length;
                ...
                return (
                  <>
                    ...
                    <div className="bg-white rounded-lg shadow-sm p-6">
                      <h3 className="text-lg font-bold text-gray-900 mb-4">All Applications for This Property</h3>
                      {propertyApps.length === 0 ? (
                        <div className="text-center py-12">
                          <Users className="w-12 h-12 text-gray-400 mx-auto mb-3" />
                          <p className="text-gray-600">No applications for this property yet</p>
                        </div>
                      ) : (
                              ...
                              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                                ...
                                <div className="flex flex-col gap-2">
                                  ...
                                  {app.status === 'rejected' && (
                                    <button 
                                      onClick={() => handleApplicationStatus(app.id, 'approved')} 
                                      className="bg-green-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-green-700 transition flex items-center justify-center gap-1.5 text-sm"
                                    >
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleAddProperty} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Campus Intake</label>
                <input
                  type="text"
                  placeholder="e.g., APK, DFC, SWC"
                  value={addForm.campus_intake}
                  onChange={e => setAddForm({ ...addForm, campus_intake: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                  required
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Space per Student (m²)</label>
                  <input
                    type="number"
                    step="0.1"
                    placeholder="15.5"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  <Upload className="inline w-4 h-4 mr-1" /> Property Images (max 5)
                </label>
                <input
                  type="file"
                  accept="image/*"
              ...
              <div className="flex gap-3 pt-4">
                <button
                  type="submit"
                  className="flex-1 bg-orange-600 text-white py-2.5 rounded-lg font-medium hover:bg-orange-700 transition flex items-center justify-center gap-2"
                >
                  <Plus className="w-5 h-5" /> Add Property
                </button>
                <button
            ...
            <form onSubmit={handleEdit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  value={editForm.campus_intake}
                  onChange={e => setEditForm({ ...editForm, campus_intake: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                  required
                />
              </div>
            ...
            <div className="p-6 max-h-[calc(90vh-8rem)] overflow-y-auto">
              ...
              <div className="bg-gradient-to-r from-orange-50 to-red-50 rounded-xl p-6 mb-6">
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    <p className="text-sm font-semibold text-gray-600 mb-1">Student Name</p>
                    <p className="text-lg font-bold text-gray-900">{viewingDocs.student_name}</p>
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-gray-600 mb-1">Email Address</p>
                    <p className="text-base text-gray-900 flex items-center gap-2">
                      <Mail className="w-4 h-4 text-orange-600" />
                      {viewingDocs.student_email}
                    </p>
                  </div>
                  ...
                  {viewingDocs.student_number && (
                    <div>
                      <p className="text-sm font-semibold text-gray-600 mb-1">Student Number</p>
                      <p className="text-base text-gray-900 flex items-center gap-2">
                  ...
                  <div>
                    ...
                    {getStatusBadge(viewingDocs.status)}
                  </div>
                  {viewingDocs.funding_approved !== undefined && (
                    <div>
                      <p className="text-sm font-semibold text-gray-600 mb-1">Funding Status</p>
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold ${viewingDocs.funding_approved ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                        {viewingDocs.funding_approved ? '✓ Approved' : '✗ Not Approved'}
                      </span>
                    </div>
                  )}
                </div>
                {viewingDocs.notes && (
                  <div className="mt-4 pt-4 border-t border-orange-200">
                    <p className="text-sm font-semibold text-gray-600 mb-1">Application Notes</p>
                    <p className="text-base text-gray-800 italic">"{viewingDocs.notes}"</p>
                  </div>
                )}
              </div>
                  ...
                  <div className="bg-gradient-to-r from-blue-50 to-blue-100 px-5 py-4 border-b-2 border-gray-200">
                    <div className="flex items-center justify-between">
                      ...
                      {viewingDocs.proof_of_registration && (
                        <a 
                          href={`http://localhost:8000${viewingDocs.proof_of_registration}`}
                          target="_blank"
                  ...
                  <div className="p-5 bg-gray-50">
                    {viewingDocs.proof_of_registration ? (
                      <div className="bg-white rounded-lg p-2 shadow-sm">
                        <embed 
                          src={`http://localhost:8000${viewingDocs.proof_of_registration}`} 
                ...
                <div className="border-2 border-gray-200 rounded-xl overflow-hidden">
                  <div className="bg-gradient-to-r from-green-50 to-green-100 px-5 py-4 border-b-2 border-gray-200">
                    <div className="flex items-center justify-between">
                      ...
                      {viewingDocs.id_copy && (
                        <a 
                          href={`http://localhost:8000${viewingDocs.id_copy}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-orange-600 hover:text-orange-700 flex items-center gap-1.5 text-sm font-semibold bg-white px-4 py-2 rounded-lg shadow-sm transition"
                        >
                  ...
                  <div className="p-5 bg-gray-50">
                    {viewingDocs.id_copy ? (
                      <div className="bg-white rounded-lg p-2 shadow-sm">
                        <embed 
                          src={`http://localhost:8000${viewingDocs.id_copy}`} 
                          type="application/pdf"
                          className="w-full h-96 rounded-lg"
                        />
                      </div>
